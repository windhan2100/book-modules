    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h2 class="sub-header">${cityName }订单查询:</h2>
          <%--添加查询开始 --%>
          	<form action="/order/query" id="query_select" method="post">
			<table id="condtion">
				<tr height="40px">
					<td align="right" width="100px">选择分店：</td>
					<td style="background-color: #FFFFFF">
						<select required="required" id="shopId" name="shopId" style="width: 150px;">
					...
					<td align="right" width="100px">选择套餐：</td>
					<td style="background-color: #FFFFFF">
						<select required="required" id="typeId" name="typeId" style="width: 150px;">
							<option value="0">--选择套餐--</option>
							<c:if test="${type != null &&  fn:length(type) > 0}">
								<c:forEach items="${type }" var="item" varStatus="status">
									<option value="${item.id }" <c:if test="${item.id == typeId }">selected="selected"</c:if> >${item.typeName }</option>
								</c:forEach>
							</c:if>
						</select>
					</td>
					<td align="right" width="100px">支付方式：</td>
					<td style="background-color: #FFFFFF">
						<select required="required" id="payId" name="payId" style="width: 150px;">
							<option value="0">--支付方式--</option>
							<c:if test="${payType != null &&  fn:length(payType) > 0}">
								<c:forEach items="${payType }" var="item" varStatus="status">
									<option value="${item.id }" <c:if test="${item.id == payId }">selected="selected"</c:if> >${item.payName }</option>
								</c:forEach>
							</c:if>
						</select>
					</td>
					<td align="right" width="100px">订单状态：</td>
					<td style="background-color: #FFFFFF">
						<select required="required" id="statusId" name="statusId" style="width: 150px;">
							<option value="-1">--选择状态--</option>
							<c:if test="${status != null &&  fn:length(status) > 0}">
								<c:forEach items="${status }" var="item" varStatus="status">
									<option value="${item.id }" <c:if test="${item.id == statusId }">selected="selected"</c:if> >${item.statusName }</option>
								</c:forEach>
							</c:if>
						</select>
					</td>
				...
				<tr height="40px">
					...
					<td style="background-color: #FFFFFF">
						<input type="text" style="width: 150px;" id="startDate" name="startDate" placeholder="开始日期" value="${startDate }"/>
					</td>
					<td align="right" width="100px">结束日期：</td>
					<td style="background-color: #FFFFFF">
						<input type="text" style="width: 150px;" id="endDate" name="endDate" placeholder="结束日期" value="${endDate }"/>
					</td>
					<td align="right" width="100px">预约手机：</td>
					<td style="background-color: #FFFFFF">
						<input type="number" id="phone" style="width: 150px;" name="phone" placeholder="无须使用其他条件" value="${phone }"/>
					</td>
			   </tr>
			   <tr height="40px">
